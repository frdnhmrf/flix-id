...
class FirebaseUserRepository implements UserRepository {
  ...
  Future<Result<int>> getUserBalance({required String uid}) async {
    try {
      ...
      return Result.failed(e.message ?? 'User not found');
    }
  }
  ...
  Future<Result<User>> uploadProfilePicture(
    ...
    try {
      ...
      var updatedImageFile =
          await updateUser(user: user.copyWith(photoUrl: downloadUrl));

      if (updatedImageFile.isSuccess) {
        return Result.success(updatedImageFile.resultValue!);
      } else {
        return Result.failed(updatedImageFile.errorMessage!);
      }
    } catch (e) {
